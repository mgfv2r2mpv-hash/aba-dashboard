...
function clientHue(name) { return Math.round((indexOf(clientIndex, name) * GOLDEN_ANGLE) % 360); }
function staffHue(name) { return Math.round((indexOf(staffIndex, name) * GOLDEN_ANGLE) % 360); }
...
export function clientDarkBorder(name) {
    if (!name)
        return '#9ca3af';
