...
// full pass.
import { computeOneClientCompliance, computeOneTechCompliance, monthPeriod, overlapHours, } from './compliance';
...
export function buildCache(data, now = new Date()) {
    const period = monthPeriod(now);
    const clients = new Map();
    const techs = new Map();
    for (const c of data.clients)
        clients.set(c.id, computeOneClientCompliance(data, c, period, now));
    for (const t of data.technicians)
        techs.set(t.id, computeOneTechCompliance(data, t, period, now));
...
export function recomputeCache(prev, oldData, newData, changes, now = new Date()) {
    ...
    for (const id of clientIds) {
        ...
        if (client)
            clients.set(id, computeOneClientCompliance(newData, client, period, now));
        else
            clients.delete(id);
    }
    for (const id of techIds) {
        ...
        if (tech)
            techs.set(id, computeOneTechCompliance(newData, tech, period, now));
        else
            techs.delete(id);
    }
...
export function summarize(cache, data) {
    ...
    const worst = red > 0 ? 'red' : yellow > 0 ? 'yellow' : 'green';
    return { red, yellow, worst };
}
//# sourceMappingURL=complianceCache.js.map