...
export default function AdminPanel({ data, onDataChange, onImportFile, onRerunWizard, onDownload, onClearData, aiSettings, onSaveAISettings, onClearKey, onRequestUnlock, faceIdAvailable, faceIdEnabled, biometryLabel, onToggleFaceId }) {
...
function ClientCard({ client, technicians, saving, onChange, onRemove }) {
    ...
    const [editing, setEditing] = useState(false);
    const [collapsed, setCollapsed] = useState(true);
    const noStaff = !(technicians ?? []).some(t => t.assignments.some(a => a.clientId === client.id || a.clientId === client.name));
    const commitMax = () => {
...
function SettingsEditor({ settings, saving, onSave, onImportFile, onRerunWizard, onDownload, onClearData, aiSettings, onSaveAISettings, onClearKey, onRequestUnlock, faceIdAvailable, faceIdEnabled, biometryLabel, onToggleFaceId }) {
    ...
    const [model, setModel] = useState(aiSettings?.model ?? 'claude-sonnet-4-6');
    ...
    const save = async () => {
        ...
        if (ok !== false) {
            ...
            if (replacingKey && apiKey.trim()) {
                const newSettings = { apiKey: apiKey.trim(), model };
    ...
    return (_jsxs("div", { style: { maxWidth: 640 }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', gap: 8, flexWrap: 'wrap' }, children: [_jsx("h3", { style: { fontSize: '18px', fontWeight: 'bold' }, children: "SAssi App Settings" }), saveBar] }), !keySavedState && aiSettings && onSaveAISettings && (_jsx(AISettingsSection, { model: model, onModelChange: setModel, hasExistingKey: hasExistingKey, replacingKey: replacingKey, apiKey: apiKey, onApiKeyChange: setApiKey, showKey: showKey, onShowKeyChange: setShowKey, onReplaceKey: async () => {
                    ...
                    setUnlockError(null);
                }, onClearKey: onClearKey, unlockError: unlockError })), faceIdAvailable !== undefined && (_jsx(SettingsSection, { title: "App Lock", children: _jsxs("div", { style: { padding: '12px', backgroundColor: '#f9fafb', borderRadius: '6px', fontSize: '13px' }, children: [_jsx("p", { style: { marginBottom: '12px', color: '#6b7280', fontSize: '12px' }, children: "A PIN locks the app on launch. There is no recovery if you forget it." }), _jsx("button", { style: {
                                padding: '8px 12px', border: '1px solid #d1d5db', borderRadius: '6px',
                                background: 'white', cursor: 'pointer', fontSize: '13px',
                            }, children: "Change PIN" }), faceIdAvailable && (_jsxs("label", { style: { display: 'flex', alignItems: 'center', gap: '8px', marginTop: '12px', cursor: 'pointer' }, children: [_jsx("input", { type: "checkbox", checked: faceIdEnabled || false, onChange: (e) => onToggleFaceId?.(e.target.checked) }), _jsxs("span", { style: { fontSize: '13px' }, children: ["Unlock with ", biometryLabel || 'Face ID'] })] }))] }) })), _jsx(SettingsSection, { title: "Schedule Password (optional)", children: _jsxs("div", { style: { padding: '12px', backgroundColor: '#f9fafb', borderRadius: '6px', fontSize: '13px' }, children: [_jsx("p", { style: { marginBottom: '12px', color: '#6b7280', fontSize: '12px' }, children: "Encrypts your downloaded schedule file. Leave blank for a normal readable file." }), hasExistingPw && !changingPw ? (_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '10px' }, children: [_jsx("span", { style: { fontSize: '13px', color: '#374151' }, children: "\uD83D\uDD12 Password is set." }), _jsx("button", { onClick: () => { setChangingPw(true); setPwError(null); }, style: {
                                        padding: '6px 12px', border: '1px solid #d1d5db', borderRadius: '6px',
                                        background: 'white', cursor: 'pointer', fontSize: '13px',
                                    }, children: "Change\u2026" })] })) : (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '8px' }, children: [hasExistingPw && (_jsx("input", { type: showPw ? 'text' : 'password', placeholder: "Current password", value: currentPw, onChange: (e) => { setCurrentPw(e.target.value); setPwError(null); }, autoComplete: "off", style: { padding: '8px 12px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px' } })), _jsxs("div", { style: { display: 'flex', gap: '8px' }, children: [_jsx("input", { type: showPw ? 'text' : 'password', placeholder: hasExistingPw ? 'New password (blank to remove)' : 'Leave blank for no encryption', value: newPw, onChange: (e) => { setNewPw(e.target.value); setPwError(null); }, autoComplete: "off", style: { flex: 1, padding: '8px 12px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px' } }), _jsx("button", { onClick: () => setShowPw(!showPw), style: {
                                                padding: '8px 12px', border: '1px solid #d1d5db', borderRadius: '6px',
                                                background: 'white', cursor: 'pointer', fontSize: '13px',
                                            }, children: showPw ? 'Hide' : 'Show' })] }), pwError && _jsx("p", { style: { fontSize: '12px', color: '#dc2626', margin: 0 }, children: pwError })] }))] }) }), _jsxs(SettingsSection, { title: "Supervision targets", children: [_jsx(NumField, { label: "Per-case Company Min. (% of Direct Service Hours)", value: directPct, onChange: setDirectPct, suffix: "%", defaultValue: 5 }), _jsx(NumField, { label: "Per-Credentialed BT Company Min. (% of Direct Service Hours)", value: rbtPct, onChange: setRbtPct, suffix: "%", defaultValue: 5, hint: "BACB floor is 5%." }), _jsx(NumField, { label: "Per-BT Company Min. (% of Direct Service Hours)", value: techPct, onChange: setTechPct, suffix: "%", placeholder: "\u2014" }), _jsx(NumField, { label: "Per-Credentialed BT Min. Contacts Count", value: minContacts, onChange: setMinContacts, suffix: "contacts", defaultValue: 2, hint: "BACB cadence requires at least 2/month." }), _jsx(NumField, { label: "Per-BT Company Min. Monthly Contacts Count", value: techMinContacts, onChange: setTechMinContacts, suffix: "contacts", defaultValue: 1 }), _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: 6 }, children: [_jsxs("label", { style: { display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }, children: [_jsx("input", { type: "checkbox", checked: pendingUncheck ? true : contactsSeparateDays, onChange: e => {
                                            if (!e.target.checked) {
                    ...
                    setUnlockError(null);
                                    ...
                                    border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 13, fontWeight: 600,
                                }, children: "\u2193 Download schedule" }))] }), onClearData && (_jsxs("div", { style: { marginTop: 12, paddingTop: 12, borderTop: '1px solid #e5e7eb' }, children: [_jsxs("p", { style: { fontSize: '12px', color: '#6b7280', margin: '0 0 6px' }, children: ["Clearing wipes the schedule loaded in the app. If you haven't saved your work, ", _jsx("strong", { children: "download it first" }), " \u2014 this can't be undone."] }), onDownload && (_jsx("div", { style: { marginBottom: 8 }, children: _jsx("button", { onClick: onDownload, style: {
                                        background: 'none', border: 'none', padding: 0, cursor: 'pointer',
                                        fontSize: 12, color: '#b91c1c', textDecoration: 'underline', fontWeight: 600,
                                    }, children: "\u2193 Download schedule first" }) })), _jsx("button", { onClick: onClearData, style: {
...
function AISettingsSection({ model, onModelChange, hasExistingKey, replacingKey, apiKey, onApiKeyChange, showKey, onShowKeyChange, onReplaceKey, onClearKey, unlockError }) {
    return (_jsxs(SettingsSection, { title: "AI Integration", children: [_jsxs("div", { children: [_jsx("label", { style: { display: 'block', fontWeight: '600', marginBottom: '8px', fontSize: '13px' }, children: "Claude Model" }), _jsx("div", { style: { display: 'flex', flexDirection: 'column', gap: '8px' }, children: MODEL_OPTIONS.map(opt => (_jsxs("label", { style: {
                                display: 'flex',
                                gap: '10px',
                                padding: '10px',
                                ...
                                backgroundColor: model === opt.value ? '#eff6ff' : 'white',
                            }, children: [_jsx("input", { type: "radio", name: "model", value: opt.value, checked: model === opt.value, onChange: () => onModelChange(opt.value), style: { marginTop: '4px' } }), _jsxs("div", { children: [_jsx("div", { style: { fontWeight: '600', fontSize: '13px' }, children: opt.label }), _jsx("div", { style: { fontSize: '12px', color: '#6b7280' }, children: opt.description })] })] }, opt.value))) })] }), _jsxs("div", { children: [_jsx("label", { style: { display: 'block', fontWeight: '600', marginBottom: '8px', fontSize: '13px' }, children: "Claude API Key" }), hasExistingKey && !replacingKey ? (_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }, children: [_jsx("span", { style: { fontSize: '13px', color: '#374151' }, children: "\uD83D\uDD12 API key is set." }), _jsx("button", { onClick: onReplaceKey, style: {
                                    padding: '6px 12px', border: '1px solid #d1d5db', borderRadius: '6px',
                                    background: 'white', cursor: 'pointer', fontSize: '13px',
                                }, children: "Replace\u2026" }), onClearKey && (_jsx("button", { onClick: () => { onClearKey(); onApiKeyChange(''); }, style: {
                                    padding: '6px 12px',
                                    background: '#fee2e2',
                                    color: '#dc2626',
                                    ...
                                    cursor: 'pointer',
                                    fontSize: '13px',
                                }, children: "Clear stored key" }))] })) : (_jsxs("div", { style: { display: 'flex', gap: '8px' }, children: [_jsx("input", { type: showKey ? 'text' : 'password', value: apiKey, onChange: (e) => onApiKeyChange(e.target.value), placeholder: hasExistingKey ? 'Enter a new key' : 'sk-ant-...', autoComplete: "off", style: {
                                    flex: 1,
                                    padding: '8px 12px',
                                ...
                                } }), _jsx("button", { onClick: () => onShowKeyChange(!showKey), style: {
                                    padding: '8px 12px',
                                    border: '1px solid #d1d5db',
                                    borderRadius: '6px',
                                    background: 'white',
                                    cursor: 'pointer',
                                    fontSize: '13px',
                                }, children: showKey ? 'Hide' : 'Show' })] })), unlockError && (_jsx("p", { style: { fontSize: '12px', color: '#dc2626', marginTop: '6px' }, children: unlockError })), _jsx("p", { style: { fontSize: '12px', color: '#6b7280', marginTop: '6px' }, children: "Your key is sent per-request via header and never stored on the server." })] })] }));
}
