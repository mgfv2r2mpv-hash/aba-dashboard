...
function AuthsTab({ data, savingId, onUpsertAuth, onRemoveAuth, onUpsertUsage, onRemoveUsage }) {
    ...
    return (_jsxs("div", { children: [_jsx("h3", { style: { marginBottom: '8px', fontSize: '18px', fontWeight: 'bold' }, children: "Authorizations" }), _jsxs("p", { style: { fontSize: '13px', color: '#6b7280', marginBottom: '16px' }, children: ["Set the authorized ", _jsx("strong", { children: "weekly rates" }), " (direct / supervision / parent-training / case-planning) \u2014 what the correction engine paces against; supervision \u2248 20% of direct is the insurer cap. Span-total buckets below remain for tracking + the per-auth reassessment block. Use manual entries for hours delivered before adopting this system (adopt-forward). The end date is the service / makeup cliff."] }), _jsxs("div", { style: { display: 'grid', gap: '16px' }, children: [data.clients.map(client => {
                        const clientAuths = auths.filter(a => a.clientId === client.id);
                        return (_jsxs("div", { style: cardStyle, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }, children: [_jsx("span", { style: { fontWeight: 600, fontSize: '15px' }, children: client.name }), _jsx("button", { onClick: () => addAuthFor(client.id), style: chipBtn, children: "+ Add authorization" })] }), clientAuths.length === 0 ? (_jsx("p", { style: { fontSize: '12px', color: '#9ca3af', fontStyle: 'italic', marginTop: 8 }, children: "No authorization on file." })) : clientAuths.map(auth => (_jsx(AuthCard, { data: data, auth: auth, saving: savingId === auth.id, onChange: (patch) => onUpsertAuth({ ...auth, ...patch }), onRemove: () => onRemoveAuth(auth.id), onUpsertUsage: onUpsertUsage, onRemoveUsage: onRemoveUsage }, auth.id)))] }, client.id));
...
function AuthCard({ data, auth, saving, onChange, onRemove, onUpsertUsage, onRemoveUsage }) {
    ...
    return (_jsxs("div", { style: { marginTop: 10, padding: '10px 12px', border: '1px solid #e5e7eb', borderRadius: 6, background: 'white' }, children: [_jsxs("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end' }, children: [_jsxs("label", { style: { display: 'flex', flexDirection: 'column', gap: 4, flex: '1 1 140px', minWidth: 0 }, children: [_jsx("span", { style: { fontSize: 11, fontWeight: 600, color: '#374151' }, children: "Auth label / number" }), _jsx("input", { value: label, onChange: e => setLabel(e.target.value), onBlur: () => { if (label !== (auth.label || ''))
                                    onChange({ label: label || undefined }); }, placeholder: "optional", style: inputStyle })] }), _jsxs("label", { style: { display: 'flex', flexDirection: 'column', gap: 4 }, children: [_jsx("span", { style: { fontSize: 11, fontWeight: 600, color: '#374151' }, children: "Start" }), _jsx("input", { type: "date", value: auth.startDate, onChange: e => onChange({ startDate: e.target.value }), style: inputStyle })] }), _jsxs("label", { style: { display: 'flex', flexDirection: 'column', gap: 4 }, children: [_jsx("span", { style: { fontSize: 11, fontWeight: 600, color: '#374151' }, children: "End (cliff)" }), _jsx("input", { type: "date", value: auth.endDate, onChange: e => onChange({ endDate: e.target.value }), style: inputStyle })] }), _jsx("button", { onClick: onRemove, style: dangerBtn, children: "Remove" })] }), _jsx("p", { style: { fontSize: 12, fontWeight: 600, color: cliffColor, marginTop: 6 }, children: usage.daysLeft < 0 ? `Expired ${-usage.daysLeft} day(s) ago` : `${usage.daysLeft} day(s) until auth ends` }), saving && _jsx("p", { style: { fontSize: 11, color: '#3b82f6' }, children: "Saving\u2026" }), _jsxs("div", { style: { marginTop: 8, paddingTop: 8, borderTop: '1px dashed #e5e7eb' }, children: [_jsxs("p", { style: { fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 6 }, children: ["Authorized weekly rates ", _jsx("span", { style: { fontWeight: 400, color: '#9ca3af' }, children: "(supervision \u2248 20% of direct = the cap)" })] }), _jsx("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap' }, children: [['direct', 'Direct'], ['supervision', 'Supervision'], ['parentTraining', 'Parent trng'], ['casePlanning', 'Case plan']].map(([key, lbl]) => (_jsxs("label", { style: { display: 'flex', flexDirection: 'column', gap: 2 }, children: [_jsxs("span", { style: { fontSize: 10, color: '#6b7280' }, children: [lbl, " h/wk"] }), _jsx("input", { type: "number", step: "0.5", min: "0", inputMode: "decimal", defaultValue: auth.weekly?.[key] !== undefined ? String(auth.weekly[key]) : '', onBlur: e => {
                                    ...
                                    }, placeholder: "\u2014", style: { ...inputStyle, width: 64 } })] }, key))) }), _jsxs("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }, children: [_jsxs("label", { style: { display: 'flex', flexDirection: 'column', gap: 2 }, children: [_jsx("span", { style: { fontSize: 10, color: '#6b7280' }, children: "Report final due (insurer)" }), _jsx("input", { type: "date", value: auth.reportFinalDue || '', onChange: e => onChange({ reportFinalDue: e.target.value || undefined }), style: { ...inputStyle, width: 150 } })] }), _jsxs("label", { style: { display: 'flex', flexDirection: 'column', gap: 2 }, children: [_jsx("span", { style: { fontSize: 10, color: '#6b7280' }, children: "Report draft due (internal)" }), _jsx("input", { type: "date", value: auth.reportDraftDue || '', onChange: e => onChange({ reportDraftDue: e.target.value || undefined }), style: { ...inputStyle, width: 150 } })] })] })] }), _jsx("div", { style: { marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }, children: AUTH_BUCKETS.map(({ key, label: bLabel }) => {
