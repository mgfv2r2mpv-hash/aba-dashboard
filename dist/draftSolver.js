...
function isSticky(a, nowMs) {
    return isActive(a) && !a.technician && a.status !== 'completed' && ms(a.startTime) >= nowMs;
}
function windowsCover(windows, startMin, endMin) {
    if (!Array.isArray(windows) || windows.length === 0)
        return true; // unconfigured = not faulted
...
function focusedConflicts(data, weekStartMs) {
    ...
    for (const a of sessions) {
        ...
        if (blackouts.some(b => b.date === date && ((client && b.entityType === 'client' && b.entityId === client.id) ||
...
function relocate(data, appt, weekStartMs, nowMs) {
    ...
    for (const slot of slots) {
        ...
        if (start === appt.startTime && end === appt.endTime)
            continue; // no-op slot
...
function reshuffleMobile(base, weekStartMs, nowMs) {
    let working = { ...base, appointments: base.appointments.map(a => ({ ...a })) };
    const movedIds = new Set();
    for (let round = 0; round < MAX_ROUNDS; round++) {
        const conflicts = focusedConflicts(working, weekStartMs);
        if (conflicts.length === 0)
            return { data: working, movedIds, clean: true };
        // Find a mobile session inside any conflict whose relocation strictly
        // reduces the conflict count.
        let improved = false;
        outer: for (const c of conflicts) {
...
function bcbaBillable(data, weekStartMs) {
    let total = 0;
...
function buildChoices(data, conflicts, nowMs) {
    const choices = [];
    const seen = new Set();
    const label = (a) => {
...
export function solveDraft(base, ops, now, settings) {
    ...
    for (const op of ops) {
        ...
        if (iso)
            weekSet.add(weekRange(new Date(iso)).start.getTime());
    }
    ...
    const everyConflictHasChoice = remaining.every(c => c.kind === 'double-book' && c.ids.some(id => choices.some(ch => ch.appointmentId === id)));
...
//# sourceMappingURL=draftSolver.js.map