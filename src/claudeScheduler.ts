...
import { ScheduleData, Appointment, WishRequest, WishSolution, WishOp, FixItOptions } from './types';
import {
...
export class ClaudeScheduler {
  ...
  // `baseURL` routes the request through a same-origin proxy instead of straight
  // to api.anthropic.com. The web portal uses it: the proxy holds the real key as
  // a server secret and re-checks the payload, so the browser carries no key at
  // all and the portal's CSP stays `connect-src 'self'`. The iOS app leaves it
  // unset - a WebView has no origin to proxy through - and keeps calling direct
  // with the user's own key. Either way the anonymizer runs HERE, in the client,
  // because the client is the only side that ever knows the real names.
  constructor(apiKey: string, data: ScheduleData, model: ClaudeModel = DEFAULT_MODEL, baseURL?: string) {
    ...
    // no client/tech names are included in the prompt (see containsRawNames).
    this.client = new Anthropic({ apiKey, dangerouslyAllowBrowser: true, ...(baseURL ? { baseURL } : {}) });
    this.data = data;
    this.model = model;
  ...
  buildSassiSystem(): string {
    ...
    const tok = (m: Map<string, string>, id: string, name: string, kind: 'CLIENT' | 'TECH') => entityToken(m, id, name, kind);
    const aptTok = (id: string) => entityToken(this.anonMap.appointments, id, null, 'APT');
    ...
    const caseLines = ctx.cases.map(c => {
      const clientTok = tok(this.anonMap.clients, c.clientId, c.clientName, 'CLIENT');
    ...
    const windowLines = ctx.directWindows.map(w => {
      const clientTok = tok(this.anonMap.clients, w.clientId, w.clientName, 'CLIENT');
      const techPart  = w.techName ? ` [${tok(this.anonMap.technicians, w.techId || '', w.techName, 'TECH')}]` : '';
      return `  ${clientTok} ${aptTok(w.appointmentId)} ${w.start.slice(0, 16).replace('T', ' ')}–${w.end.slice(11, 16)}${techPart}`;
    });
    const blockerLines = ctx.blockers.map(b => `  ${tok(this.anonMap.clients, b.clientId, b.clientName, 'CLIENT')}: ${b.blocker}`);
...
ISO times are local (no timezone suffix). Verify: every op start ≥ NOW; no two BCBA items overlap; tokens exist in CLIENTS/TECHNICIANS; skip malformed tokens.`;
  ...
  buildFixItPrompt(options: FixItOptions, conflicts: string[]): string {
    ...
    const focusTok = focusClient
      ? entityToken(this.anonMap.clients, focusClient.id, focusClient.name, 'CLIENT')
      : undefined;
    ...
    const clientGaps = computeClientCompliance(this.data, period)
      ...
      .map(r => {
        const token = entityToken(this.anonMap.clients, r.client.id, r.client.name, 'CLIENT');
    ...
    const techGaps = computeTechCompliance(this.data, period)
      ...
      .map(r => {
        const token = entityToken(this.anonMap.technicians, r.tech.id, r.tech.name, 'TECH');
    ...
    const supWindowLines = supWindows.map(w => {
      const ct = tok(this.anonMap.clients, w.clientId, w.clientName, 'CLIENT');
      const tt = w.techName ? ` [BT:${tok(this.anonMap.technicians, w.techId || '', w.techName, 'TECH')}]` : '';
    ...
    const blockedDiagLines = diagnostics
      ...
      .map(d => {
        const ct = tok(this.anonMap.clients, d.clientId, d.clientName, 'CLIENT');
        return `${ct}: ${d.blocker}`;
      });
