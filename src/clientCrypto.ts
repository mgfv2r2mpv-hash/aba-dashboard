...
export async function obfuscateKey(plaintext: string): Promise<string> {
  return encryptString(plaintext, APP_OBFUSCATION_PASSPHRASE);
}

export async function deobfuscateKey(blob: string): Promise<string> {
...
export function isEncryptedSchedule(bytes: Uint8Array): boolean {
...
export async function decryptBytes(blob: Uint8Array, password: string): Promise<Uint8Array> {
  if (!startsWithMagic(blob)) throw new Error('Not an encrypted schedule file');
