...
interface LockScreenProps {
  mode: 'create' | 'unlock';
  // Create flow: persist the chosen PIN. Unlock flow: validate, return true on
  // success (the screen shows an error and clears on false).
  onCreate?: (pin: string) => Promise<void> | void;
  onVerify?: (pin: string) => Promise<boolean>;
  // Optional biometric unlock (only passed when enabled + available). Returns
...
export default function LockScreen({ mode, onCreate, onVerify, onBiometric, biometricAuto }: LockScreenProps) {
  ...
  const submitCreate = async () => {
    if (pin.length < PIN_MIN) { setError(`Use at least ${PIN_MIN} digits.`); return; }
    if (stage === 'enter') {
      setStage('confirm');
      return;
    }
  ...
  const submitUnlock = async () => {
    if (pin.length < PIN_MIN) { setError('Enter your PIN.'); return; }
    setBusy(true);
    setError(null);
    try {
  ...
  const setValue = (v: string) => {
    if (!isDigits(v) || v.length > PIN_MAX) return;
    setError(null);
    if (mode === 'create' && stage === 'confirm') setConfirm(v);
    else setPin(v);
  };
  ...
  return (
    <div style={{
      ...
      <form onSubmit={onSubmit} style={{ width: '100%', maxWidth: 320, textAlign: 'center' }}>
        <div style={{ fontSize: 40, marginBottom: 12 }}>🔒</div>
        <h1 style={{ fontSize: 22, fontWeight: 700, margin: '0 0 8px' }}>{title}</h1>
        <p style={{ fontSize: 13, color: '#9ca3af', margin: '0 0 20px', lineHeight: 1.4 }}>{subtitle}</p>
