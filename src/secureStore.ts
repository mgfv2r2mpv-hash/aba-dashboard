...
export async function readBlob(name: string): Promise<string | null> {
  ...
  try { return localStorage.getItem(path); } catch { return null; }
}

export async function writeBlob(name: string, value: string): Promise<void> {
  const path = PREFIX + name;
  if (Capacitor.isNativePlatform()) {
    await Filesystem.writeFile({ path, directory: Directory.Data, encoding: Encoding.UTF8, data: value });
    return;
  }
  try { localStorage.setItem(path, value); } catch { /* ignore */ }
}

export async function deleteBlob(name: string): Promise<void> {
  const path = PREFIX + name;
  if (Capacitor.isNativePlatform()) {
    try { await Filesystem.deleteFile({ path, directory: Directory.Data }); } catch { /* already gone */ }
    return;
  }
  try { localStorage.removeItem(path); } catch { /* ignore */ }
}
