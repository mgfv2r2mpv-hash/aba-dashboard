...
export interface SessionFlags {
  ...
  /** Per-(tech, client) consecutive completed-session count (direct sessions only). */
  clientCompletedStreak?: number;
...
export function computeSessionFlags(
  ...
  // Per-(tech, client) streak for direct sessions (P3e).
  const streakByTechClient = new Map<string, number>();
  ...
  for (const apt of sorted) {
    ...
    // ── Completed streak (per tech overall + per tech+client for directs) ─────
    if (apt.technician) {
      if (apt.status === 'completed') {
        const streak = (streakByTech.get(apt.technician) ?? 0) + 1;
        streakByTech.set(apt.technician, streak);
        flags.completedStreak = streak;
        // Per-(tech, client) streak for direct sessions only.
        if (apt.client && apt.type === 'client-session') {
          const tcKey = `${apt.technician}|${apt.client}`;
          const cs = (streakByTechClient.get(tcKey) ?? 0) + 1;
          streakByTechClient.set(tcKey, cs);
          flags.clientCompletedStreak = cs;
        }
        ...
        if (apt.client) streakByTechClient.set(`${apt.technician}|${apt.client}`, 0);
      }
